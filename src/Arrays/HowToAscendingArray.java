...
public class HowToAscendingArray {
    //(Array) Sort Ascending
    //Write a return method that can sort an int array in Ascending order without using the sort method of the Arrays class
    public static void main(String[] args) {
        int [] myArray = {-2, 34, 679, -90};
        System.out.println(Arrays.toString(ascendingOrder(myArray)));
    }
    public static int[] ascendingOrder(int[] param1) { //we need to create a method one int [] parameter
        for (int i = 0; i < param1.length; i++) { //we create one for loop starts with 0 ends with param1.length
            for (int j = i + 1; j < param1.length; j++) { //then we create nested for loop starts with i+1 ends with param1.length
                ...
                if (param1[i] > param1[j]) { //then we create if condition if param1[i] bigger than param1[j]
